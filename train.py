...
from lag_horizontal import LAG  # Your model
...
# ✅ Accuracy function for multi-class
...
for epoch in range(start_epoch, total_epochs + 1):
    ...
    for batch_idx, (image, label) in enumerate(train_dataloader):
        prediction = model(image)  # shape: [B, num_classes]
        tloss = loss_function(prediction, label.long())  # ✅ cast label to long for CrossEntropy
    ...
    model.eval()
    with torch.no_grad():
        for batch_idx, (img, lbl) in enumerate(val_dataloader):
            prediction = model(img)
            vloss = loss_function(prediction, lbl.long())
            val_loss += vloss.item()
    ...
    print(f'Epoch [{epoch}/{total_epochs}], Train Loss: {train_loss:.4f}, Train Acc: {t_accuracy * 100:.2f}%, Val Loss: {val_loss:.4f}, Val Acc: {v_accuracy * 100:.2f}%, Time: {time.time() - s_time:.2f}s')
